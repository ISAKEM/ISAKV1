...
if (DataStore.get("Dev-mode")) {
    try  {
		let res = await fetch(`//plugins/${getPluginsName()}/Server-Data/data/built-in_plugins/Settings.js`)
		if (res.status == 200) {
			init = (await (() => import(`//plugins/${getPluginsName()}/Server-Data/data/built-in_plugins/Settings.js`))()).init
		}
	}
